...
def left_rotate(val: int, shift: int, bits: int) -> int:
    return ((val << shift) & ((1 << bits) - 1)) | (val >> (bits - shift))

def generate_keys(key: bytes) -> List[int]:
    key64 = int.from_bytes(key, 'big')
    key56 = permute(key64, PC1, 64)
    C, D = (key56 >> 28) & 0xFFFFFFF, key56 & 0xFFFFFFF
    subkeys = []
...
def s_box_substitution(x: int) -> int:
    ...
    for i in range(8):
        block = (x >> (42 - i*6)) & 0x3F
        row = ((block & 0b100000) >> 4) | (block & 0b000001)
        col = (block >> 1) & 0b1111
        val = S_BOX[i][row][col]
        output = (output << 4) | val
    return output

def f_function(R: int, K: int) -> int:
    ER = permute(R, E, 32)
    x = ER ^ K
    s_output = s_box_substitution(x)
    return permute(s_output, P, 32)

def des_rounds(block: int, subkeys: List[int], decrypt: bool = False) -> int:
...
def des_encrypt_block(block: bytes, key: bytes) -> bytes:
    ...
    encrypted = des_rounds(block_int, key_schedule)
    return encrypted.to_bytes(8, 'big')

def des_decrypt_block(block: bytes, key: bytes) -> bytes:
    block_int = int.from_bytes(block, 'big')
    key_schedule = generate_keys(key)
    decrypted = des_rounds(block_int, key_schedule, decrypt=True)
